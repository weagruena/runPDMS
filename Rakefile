...
task :release => [:config, :backup, :make_exe] do		
end
...
task :make_exe => [:config] do
  Dir.mkdir("release") if !File.exists?("release") 
	...
	File.copy("./#{@proj}.db", "../release", true)
	File.copy("./#{@proj}.hta", "../release", true)
	File.copy("./#{@proj}.png", "../release", true)
...
task :doc => [:config] do
	...
	File.open("header.txt", "w+") do |f|
		...
		f.puts "\n-----------------------------------\n"
     Dir["*.*"].sort.each do |vb|
		if !ex.include?(File.extname(vb))
			dt = File.mtime(vb).strftime("%d.%m.%Y %H:%M")
			fline = File.readlines(vb)[0]
			f.puts "#{vb}\t\t#{dt}\t\t#{fline}" if File.extname(vb) != ".txt"
		end
	end
	end 
	...
	Dir["*"].sort.each do |vb|
		if !ex.include?(File.extname(vb))
			system(@enscript + @param1 + "../doc/#{vb}.ps #{vb}") if File.extname(vb) != ".txt"
		end
	end
	#~ # Make PDF
	Dir.chdir("../doc")
